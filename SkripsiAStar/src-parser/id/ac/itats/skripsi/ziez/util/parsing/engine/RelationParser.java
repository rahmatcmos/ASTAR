package id.ac.itats.skripsi.ziez.util.parsing.engine;

import id.ac.itats.skripsi.ziez.util.parsing.osm.Member;
import id.ac.itats.skripsi.ziez.util.parsing.osm.OSM;
import id.ac.itats.skripsi.ziez.util.parsing.osm.Relation;
...
public class RelationParser {
	...
	public static Relation parseRelation(OSM osm, Node node) {
		...
		return new Relation(osm, id, getAttribute(atts, "visible"),
				getAttribute(atts, "timestamp"), getAttribute(atts, "version"),
				getAttribute(atts, "changeset"), getAttribute(atts, "user"),
				getAttribute(atts, "uid"), getMembers(node.getChildNodes()),
	...
	private static List<Member> getMembers(NodeList children) {
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (node.getNodeName().equals("member")) {
				result.add(new Member(map.getNamedItem("type").getNodeValue(),
						map.getNamedItem("ref").getNodeValue(), map
								.getNamedItem("role").getNodeValue()));
			}
		}
