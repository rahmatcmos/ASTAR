...
public class MainActivity extends MapActivity {
	...
	public void calcPath(final double fromLat, final double fromLon,
		...
		new AsyncTask<Void, Void, AStarResponse>() {
			float time;

			protected AStarResponse doInBackground(Void... v) {
				StopWatch sw = new StopWatch().start();

				AStarRequest req = new AStarRequest(fromLat, fromLon, toLat,
						toLon);
				AStarResponse resp = astar.route(req);
				time = sw.stop().getSeconds();
				
				return resp;
			}

			protected void onPostExecute(AStarResponse resp) {
				if (!resp.hasError()) {
					log("from:" + fromLat + "," + fromLon + " to:" + toLat
							+ "," + toLon + " found path with distance:"
							+ resp.distance() / 1000f + ", nodes:"
							+ resp.points().size() + ", time:" + time + " "
							+ resp.debugInfo());
